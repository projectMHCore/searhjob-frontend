    ...
    try {
        ...
        if (!empty($updates)) {
            ...
            if ($stmt) {
                ...
                $message = 'Ошибка подготовки запроса: ' . $db->error;
            }
        } else {
...
<body>
    ...
    <nav class="navbar">
        <div class="nav-container">
            ...
            <ul class="nav-menu">                <li><a href="/frontend/index.php" class="nav-link">Головна</a></li>
                ...
                <li><a href="/frontend/profile.php" class="nav-link active">Профиль</a></li>
                <?php if ($userData['role'] === 'employer'): ?>
                    <li><a href="/frontend/my_vacancies.php" class="nav-link">Мои вакансии</a></li>
                <?php endif; ?>
            </ul>
    ...
    <main class="main-content">
        <div class="container">
            ...
            <div class="page-header">
                <?php
                $pageTitle = '✏️ Редактирование профиля';
                $subtitle = 'Обновите свою информацию';
                if ($editType === 'personal') {
                    $pageTitle = '👤 Личная информация';
                    $subtitle = 'Редактирование персональных данных';
                } elseif ($editType === 'company') {
                    $pageTitle = '🏢 Информация о компании';
                    $subtitle = 'Редактирование данных компании';
                }
                ?>
                <h1><?= $pageTitle ?></h1>
            ...
            <form method="post" class="edit-form">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="education" name="education" 
                                  placeholder="Укажите ваше образование, учебные заведения, специальность..."><?= htmlspecialchars($currentData['education'] ?? '') ?></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="skills">Навыки и компетенции</label>
                        <textarea id="skills" name="skills" 
                                  placeholder="Перечислите ваши профессиональные навыки, технологии, языки программирования..."><?= htmlspecialchars($currentData['skills'] ?? '') ?></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="about_me">О себе</label>
                        <textarea id="about_me" name="about_me" 
                                  placeholder="Расскажите о себе, своих достижениях, целях..."><?= htmlspecialchars($currentData['about_me'] ?? '') ?></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="salary_expectation">Желаемая зарплата (грн.)</label>
                        <input type="number" id="salary_expectation" name="salary_expectation" 
                               value="<?= htmlspecialchars($currentData['salary_expectation'] ?? '') ?>"
                               min="0" step="1000" placeholder="Например: 25000">
                        <div class="help-text">Указывайте сумму в гривнах (UAH)</div>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="company_description" name="company_description" 
                                  placeholder="Расскажите о деятельности компании, миссии, ценностях..."><?= htmlspecialchars($currentData['company_description'] ?? '') ?></textarea>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="company_website">Веб-сайт компании</label>
                    ...
                    <div class="form-group">
                        ...
                        <select id="company_size" name="company_size">
                            ...
                            <option value="1-10" <?= ($currentData['company_size'] ?? '') === '1-10' ? 'selected' : '' ?>>1-10 сотрудников</option>
                            <option value="11-50" <?= ($currentData['company_size'] ?? '') === '11-50' ? 'selected' : '' ?>>11-50 сотрудников</option>
                            <option value="51-200" <?= ($currentData['company_size'] ?? '') === '51-200' ? 'selected' : '' ?>>51-200 сотрудников</option>
                            <option value="201-500" <?= ($currentData['company_size'] ?? '') === '201-500' ? 'selected' : '' ?>>201-500 сотрудников</option>
                            <option value="500+" <?= ($currentData['company_size'] ?? '') === '500+' ? 'selected' : '' ?>>Более 500 сотрудников</option>
                        </select>
                    </div>
                    
                    <div class="form-group">
                        <label for="company_industry">Отрасль</label>
                        <input type="text" id="company_industry" name="company_industry" 
                               value="<?= htmlspecialchars($currentData['company_industry'] ?? '') ?>"
                               placeholder="Например: ИТ, Финансы, Торговля">
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="form-actions">
                    ...
                    <a href="/frontend/profile.php" class="btn btn-secondary">❌ Отмена</a>
                </div>
                
                <!-- Navigation Links for Employers -->
                <?php if ($userData['role'] === 'employer' && $editType !== 'full'): ?>
                <div class="navigation-links">
                    <h4>Переключить режим редактирования:</h4>
                    <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap; margin-top: 1rem;">
                        <?php if ($editType === 'personal'): ?>
                            <a href="edit_profile.php?type=company" class="btn btn-outline">🏢 Редактировать компанию</a>
                            <a href="/frontend/edit_profile.php" class="btn btn-outline">📝 Полное редактирование</a>
                        <?php elseif ($editType === 'company'): ?>
                            <a href="edit_profile.php?type=personal" class="btn btn-outline">👤 Редактировать профиль</a>
                            <a href="/frontend/edit_profile.php" class="btn btn-outline">📝 Полное редактирование</a>
                        <?php endif; ?>
                    </div>
                </div>
